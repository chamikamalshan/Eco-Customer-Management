...
export default function AllWaste(){
    ...
    useEffect(()=>{
        function getWastes() {
            axios.get("http://localhost:8070/waste/allwaste").then((res)=>{
               setWastes(res.data);
               console.log(res);
        }).catch((err)=>{
            alert(err.message);
   ...
   const deleteWaste= async(id)=>{
    console.warn(id)
    let result = await fetch(`http://localhost:8070/waste/deletewaste/${id}`,{
        method:"Delete"
    });
        ...
        <div class="card bg-dark text-white" >
           <img src="wastebg.png" class="card-img" alt="..." style={{height: '375px', filter: 'brightness(40%)'}}/>
             <div class="card-img-overlay">
              <br></br>
              <br></br>
                <h1 class="card-title" style={{fontSize: '90px'}}><b>VIEW</b> <b class="text-success">WASTE</b></h1>
                <br></br>
                <br></br>
                <br></br>
        ...
        <table class="table table-success table-striped">
            ...
            {wastes.filter((wastes) => {
                ...
                <tr key={wastes._id}>
                    ...
                    <td>Rs.{wastes.wprice}</td>
                    <td>{wastes.dsite}</td>
                    <td>
                        <a className="btn btn-success" href={`/updatewaste/${wastes._id}`}>
                            <i className="fas fa-edit"></i>&nbsp;Update
                        </a>

                        <h>    </h>
                        
                        <a className="btn btn-secondary" href="#" onClick={()=>deleteWaste(wastes._id)}>
