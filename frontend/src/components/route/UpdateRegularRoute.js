...
export default function UpdateRegularRoute(){

  const [rdayid, setRDayID] = React.useState("");
  const [rday, setRDay] = React.useState("");
  const [rroute, setRRoute] = React.useState("");
  ...
  useEffect(()=>{
    getRegularRouteDetails();
  },[])

  const getRegularRouteDetails = async ()=>{
    console.warn(params)
    let result = await fetch(`http://localhost:8070/regularroute/getrroute/${params.id}`);
    result = await result.json();
    setRDayID(result.rdayid);
    setRDay(result.rday);
    setRRoute(result.rroute);
  }

  const UpdateRegularRoute = async () =>{
    console.warn(rdayid,rday,rroute)
    let result = await fetch(`http://localhost:8070/regularroute/updaterroute/${params.id}`,{
      method:'put',
      body:JSON.stringify({rdayid,rday,rroute}),
      headers:{
        ...
        <br></br>
          ...
          <form class="row g-3 was-validated">
              <div class="col-md-6">
                  ...
                  <select class="form-select is-invalid" id="validationServer01" aria-label="Default select example" value={rdayid}
                  onChange={(e)=>{
              ...
              <div class="col-md-6">
                  ...
                  <select class="form-select is-invalid" id="validationServer01" aria-label="Default select example" value={rday}
                  onChange={(e)=>{
              ...
              <div class="col-12">
                  ...
                  <select class="form-select is-invalid" id="validationServer01" aria-label="Default select example" value={rroute}
                  onChange={(e)=>{
              ...
              <br></br>
              <Link to='/allrroute'>
              <button type="submit" class="btn btn-success" onClick={UpdateRegularRoute}>SUBMIT</button>
              </Link>
