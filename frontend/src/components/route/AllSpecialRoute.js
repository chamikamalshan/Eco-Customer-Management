...
export default function AllSpecialRoute(){
    ...
    useEffect(()=>{
        function getSpecialRoutes() {
            axios.get("http://localhost:8070/specialroute/allsroute").then((res)=>{
               setSpecialRoutes(res.data);
               console.log(res);
        }).catch((err)=>{
            alert(err.message);
        })
   ...
   const deleteSRoute= async(id)=>{
    console.warn(id)
    let result = await fetch(`http://localhost:8070/specialroute/deletesroute/${id}`,{
        method:"Delete"
    });
   ...
   const generatePDF = useReactToPrint({
    ...
    return(
        ...
        <br></br>
        <form class="d-flex">
        <input class="form-control me-2 " type="text" onChange={(e) => setsearch(e.target.value)} placeholder="Search by SID" aria-label="Search"/>
        </form>
        <br></br>
        <div>
        ...
        <table class="table table-success table-striped">
            ...
            <tbody>
            {specialroutes.filter((specialroutes) => {
                return search.toString().toLowerCase() === '' ? specialroutes: specialroutes.sid.toString().toLowerCase().includes(search);
                
            }).map((specialroutes, index) => {
                return(
                <tr key={specialroutes._id}>
                    <th scoop="row">{index+1}</th>
                    <td><a href={`/get/${specialroutes._id}`} style={{textDecoration:'none',color:'black'}}>{specialroutes.sid} </a> </td> 
                    <td>{specialroutes.sdistrict}</td>
                    <td>{specialroutes.smaincity}</td>
                    <td>{specialroutes.saddress}</td>
                    <td>
                        <a className="btn btn-success" href={`/updatesroute/${specialroutes._id}`}>
                            <i className="fas fa-edit"></i>&nbsp;Update
                        </a>

                        <h>    </h>
                        
                        <a className="btn btn-secondary" href="#" onClick={()=>deleteSRoute(specialroutes._id)}>
